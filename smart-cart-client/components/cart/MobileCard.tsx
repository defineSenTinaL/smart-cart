...
const MobileCard: React.FC<any> = ({ product, quantity }) => {

  const { authUser } = useAuth();
  const { user } = userStore();


  const { updateCart, removeFromCart } = cartStore.getState();

  const increaseQuantity = () => {
    // Directly call the action to increase the quantity
    updateCart(product.productId._id, (quantity || 0) + 1);
  };

  const decreaseQuantity = () => {
    // Directly call the action to decrease the quantity
    updateCart(product.productId._id, Math.max((quantity || 0) - 1, 0));
  };

  const removeItem = () => {
    // Directly call the action to remove the item from the cart
    removeFromCart(product.productId._id);
    toast.success("Product removed from cart!");

    try {
      removeFromCartDB(user._id, product.productId._id, authUser?.authtoken);
    } catch (error) {
